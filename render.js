...
function calcArrowTransform(radius,  gender, arcHeight) {
    ...
    return {angle: angle, rotationCenter: {x: centerX, y: centerY}};
}
...
function drawArc(draw, color, offset, sex, arcHeight, lineWidth) {
    ...
    var arcOffset = {x:  offset.x, y: offset.y};
    if(sexDirection == 0) {
    ...
    path.stroke({color: color, width: lineWidth, linecap: 'square'});
 }
 ...
 function drawArrow(draw, color, offset, sex, gender, sexChange, arcHeight, lineWidth, inside) {
     ...
     path.fill('none');
     path.stroke({color: color, width: lineWidth});
 }
